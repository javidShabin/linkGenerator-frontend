...
const ProUsers = () => {
  const [proUserList, setProUserList] = useState([]);
  ...
  useEffect(() => {
    const getUsersList = async () => {
      ...
      try {
        ...
        let findProUsers = response.data.data.users;
        setProUserList(findProUsers.filter((user) => user.isPro == true));
      } catch (error) {
        console.log(error);
      } finally {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setProUserList((prev) => prev.filter((user) => user._id !== deleteTarget._id));
  ...
  return (
    <div className="relative min-h-screen bg-[#05030e] text-white p-6 md:p-10 overflow-hidden font-inter">
      ...
      <h1 className="text-white text-3xl font-bold mb-8">Premium Users</h1>
